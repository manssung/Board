...
export default function StrategyGenerator() {
  ...
  const handleConditionClick = (condition) => {
    ...
    setSelectedConditions(prev => [...prev, { ...condition, comment: undefined, operator: 'and', openParen: '', closeParen: '' },]);
  ...
  // 괄호 토글 함수
  const handleToggleParen = (index, type) => {
    const newConditions = selectedConditions.map((item, i) => {
      if (i === index) {
        if (type === 'open') {
          return { ...item, openParen: item.openParen === '(' ? '' : '(' };
        } else {
          return { ...item, closeParen: item.closeParen === ')' ? '' : ')' };
        }
      }
      return item;
    });
...
*/
  const handleAiGenerate = async () => {
    ...
    if (newCondition) {
      setSelectedConditions(prev => [...prev, { ...newCondition, comment: undefined, openParen: '', closeParen: '' }]);
      setIsMentManuallyEdited(false);
    }
  };
            ...
            <div id="right-panel" className="panel">
              ...
              <div className="panel-content">
                <SelectedConditions
                  ...
                  onToggleParen={handleToggleParen}
                />
              </div>
            </div>
          </div>
